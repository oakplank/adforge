...
function parseImageDataUri(imageDataUri) {
    const match = imageDataUri.match(/^data:(image\/[a-zA-Z0-9.+-]+);base64,(.+)$/);
    if (!match)
        return null;
    return { mimeType: match[1], data: match[2] };
}
function hasCriticalQualityIssue(lines) {
...
export class NanoBananaClient {
    ...
    sanitizeQaModel(model) {
        if (!/^[a-zA-Z0-9._-]+$/.test(model))
            return DEFAULT_QA_MODEL;
        return model;
    }
    normalizeQualityReport(raw) {
        ...
        const issues = Array.isArray(raw?.issues)
            ? raw.issues.filter((entry) => typeof entry === 'string').slice(0, 6)
            : [];
        const correctionPrompt = typeof raw?.correctionPrompt === 'string'
    ...
    inferQualityFromTextResponse(text) {
        const normalizedText = text.trim();
        const lower = normalizedText.toLowerCase();
        const scoreMatch = normalizedText.match(/(?:score|rating)\s*[:=]?\s*(\d{1,3})/i) ||
        ...
        const lines = normalizedText
            ...
            .filter(Boolean);
        const issueLines = lines.filter((line) => /issue|artifact|problem|violation|text|watermark|split|border|letterbox/i.test(line));
        const correctionMatch = normalizedText.match(/correction(?: prompt)?\s*[:\-]\s*([^\n]+)/i) ||
    ...
    async reviewImageQuality(imageDataUri, context) {
    ...
    async generateImageWithQualityGate(input) {
        ...
        for (let attempt = 1; attempt <= MAX_QUALITY_ATTEMPTS; attempt += 1) {
            attempts = attempt;
            const image = await this.generateImage(input.prompt, input.width, input.height, attemptPrompt, input.systemPrompt, selectedModel);
