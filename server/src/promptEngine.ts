...
interface CreativeThoughtSession {
  targetAudience: string;
  decisionSummary: string;
  narrativeMoment: string;
  humanPresence: 'none' | 'hands_only' | 'partial_person' | 'full_person';
...
function resolveAudience(rawPrompt: string, description: string, category: string, brand?: BrandProfile): string {
...
function buildCreativeThoughtSession(input: {
  ...
  let humanPresence: CreativeThoughtSession['humanPresence'] = 'partial_person';
...
function buildRenderPrompt(input: {
  ...
  creativeInterpretation: CreativeInterpretation;
  interpretiveLayer: InterpretiveLayerOutput;
  thoughtSession: CreativeThoughtSession;
}): string {
  const {
    product,
    description,
